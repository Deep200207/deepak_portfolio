import project1 from "../assets/project1.png";
import project2 from "../assets/project2.png";
import project3 from "../assets/project3.png";


export const HERO_CONTENT = `I'm a passionate Full-Stack Web Developer specializing in the MERN stack — MongoDB, 
    Express.js, React.js, and Node.js. With a strong command of frontend technologies like Tailwind CSS, 
    JavaScript, and HTML, I create responsive and visually appealing user interfaces. On the backend, 
    I build scalable RESTful APIs and manage data effectively using MongoDB. I'm also skilled in Java,
     which strengthens my problem-solving and object-oriented programming capabilities. Whether it's developing 
     a modern UI or building full-stack applications from scratch.I'm constantly learning and excited to build innovative solutions for the web`;
export const ABOUT_ME = `I’m Deepak — a tech enthusiast who turned curiosity into code.
    My journey into full-stack development started with small frontend experiments
    and quickly grew into building complete web applications. Today, I work with 
    the MERN stack, combining MongoDB, Express, React, and Node.js to build fast 
    and interactive user experiences. I love crafting beautiful UIs with Tailwind
    CSS and solving backend logic with Java and JavaScript. I’m building 
    a project solo or collaborating with a team, I enjoy the process of learning, 
    building, and improving.`
export const Project=[
  {
    title:"E-commerce Website",
    image:project1,
    Link:"https://deep200207.github.io/myntra-clone/",
    description:"A E-commerce website with features like Product listing and shoping cart. ",
    technologies:["HTML","Tailwind-CSS","REACT","JavaScript"],
  },
  {
    title:"Chat App",
    image:project2,
    description:"A Real-time chat Applicatioin feature like chating ,secure chating and user Authentication.",
    technologies:["HTML","Tailwind-Css","React","Express Js","MongoDb"]
  },
  {
    title:"Student DashBoard Management",
    image:project3,
    description:"A management web app for school management system feature like delete ,update profile, marks or attendence.",
    technologies:["HTML","Tailwind-CSS","React","MongoDB","Node.js"]
  }
]

export const Contact_Us={
  address:"New-Delhi, India",
  phone:"+91 7982919871",
  email:"royannu149@gmail.com",
};
